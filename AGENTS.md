...
- Dock is for account login, mount management, settings/progress/verification.
- The Siyuan Cloud file manager tab can hand audio/video files to the sibling SiYuan Media Player plugin through `window.siyuanMediaPlayer.playMediaItem`, using the OpenList-compatible `/api/fs/get.raw_url` as the media URL. For proxied storages this is `/p/<path>`; `/d/<path>` remains a fallback download route. SiYuan Media Player must treat `/plugin/private/siyuan-cloud/p` and `/d` URLs as final playback URLs and must not re-resolve them through its own cloud-drive drivers. Image files use SiYuan's native Viewer.js image preview path.
...
- `OpenList`, `AListV3`/`AList V3`, `WebDav`, `S3`/`Doge`, `Onedrive`/`OneDrive`, `123Pan`/`123`, and `BaiduNetdisk` have first runtime adapters through SiYuan `/api/network/forwardProxy`; Baidu proxy fallback is served by the plugin's own `/d`/`/p` routes so standard media headers are preserved.
...
State is stored with `siyuan.storage` at `siyuan-cloud/state.json`.
