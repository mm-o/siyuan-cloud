...
- 适配思源 3.8.3 的文件写入流程，直接调用 `putFile` 时带上当前前端 `appId`，避免误触发覆盖通知和重复刷新。
- 统一工作空间与预览模块的思源文件写入路径，保持现有 `POST /api/file/putFile` 方式。
- 优化目录和文件加载：根据挂载驱动直接分流到本地、OpenList/AList 或内核路径，缓存挂载配置，避免文件详情重复绕回路由，并仅在确实限流时重试请求。
- 更新 123 云盘下载流程，优先使用当前 v2 下载接口，同时保留旧版双地址回退，兼容旧环境和接口不可用的情况。
