...
当前已对照 `docs/OpenList-main/drivers/*/meta.go` 做过一轮字段/config 校正：WebDav、123Pan 保持 `PreferProxy`；S3/Doge 保持 `CheckStatus` 并补齐 upstream addition 字段；OpenList 保留 `ProxyRangeOption` / `LinkCacheMode=auto` metadata；AliyundriveOpen、Quark/UC/QuarkOpen/QuarkTV/UCTV、WPS、Local 保留 upstream default root、no overwrite、no upload、only proxy、no cache/no link url 等 config 差异；普通 `Quark` 不固定 `PreferProxy`，而是在新建/更新挂载时按 OpenList `Init` 语义，仅当未启用 `use_transcoding_address` 时默认写入 `web_proxy=true`；115 Cloud 保留 OpenList `LinkCacheMode=ua` 和 `cookie`/`qrcode_token` 二选一登录语义，已接 list/get/link/basic management/details runtime，并按 `LimitRate -> WaitLimit` 在公开操作入口限速，但上传仍按 `no_upload=true` 暂不声明完成。WPS 已按 OpenList `drivers/wps` 接入 Cookie 登录校验、Personal/Business 模式、群组/文件列表、下载链接、基础管理和容量详情；上传保持 `no_upload=true`，因为 OpenList 的 WPS 上传依赖 Go 后端流式处理，而思源 JS kernel 的 base64 解码、SHA1/SHA256 校验和 forwardProxy 上传会阻塞思源运行环境。

WebDav runtime 已按 `docs/OpenList-main/drivers/webdav` 与 `docs/OpenList-main/pkg/gowebdav` 的主路径重整：内部命名保持 `readDir/stat/link/mkdirAll/renamePath/copyPath/removeAll/writeStream`，列表使用 `PROPFIND Depth:1` 并跳过 self collection，`get`/stat 使用 `PROPFIND Depth:0` 而不是 HEAD，对象名优先从 href basename 解析，`read()` 只返回 `model.Link` 风格 URL/header，不在 driver 层注入 `raw_url` 或其它 UI 字段。当前仍未等价迁移 cookie jar、SharePoint `odrvcookie`、`tls_insecure_skip_verify`、Basic/Digest 401 negotiation、409 parent retry 和 `Link(args.Redirect)` no-redirect probe；后续 WebDAV 问题优先继续对照 OpenList/gowebdav 源码修 driver，不在 FileTab/Dock 增加服务商专用补丁。
...
Quark/UC 普通版上传已对齐 OpenList `drivers/quark_uc` 的 `Put -> upPre -> upHash -> upPart -> upCommit -> upFinish` 主链路：`file/upload/pre`、`file/update/hash`、`file/upload/auth`、OSS part `PUT`、OSS `CompleteMultipartUpload` XML 和 `file/upload/finish` 的 API、字段和 `auth_meta` canonical 文本保持上游命名。S3/Doge 的 `/api/fs/link` 已补 OpenList `Link` 语义：生成 GET 预签名 URL，支持 `custom_host`、`enable_custom_host_presign`、`remove_bucket` 和 `response-content-disposition` 文件名参数；`/p` 预览仍走插件内 S3 签名读取，避免思阅/PDF.js Range 请求把预签名代理链路打成上游 403。`force_path_style` 默认值对齐 OpenList 为关闭。`GetDirectUploadInfo` 按 OpenList `PutObjectRequest.Presign` 行为生成 SigV4 query presign，并支持 `direct_upload_host` 替换；前端上传优先使用 OpenList `HttpDirect` 直传，失败或无直传时才回落到显式 base64 `/api/fs/put`，避免 multipart 在 SiYuan JS kernel 中丢失文件内容生成 0KB。WebDav `Put` 显式写 `Content-Type` 和 `Content-Length`，对齐 upstream `WriteStream` callback。
...
Quark/UC 请求会按 OpenList `request()` 行为吸收 `__puus` cookie，Quark 转码模式额外吸收 `__pus`，并通过 runtime `saveDriverStorage` 写回 addition；Quark/UC 也已接 `/member` 容量 details。QuarkOpen 上传 proof 依赖 `/open/v1/user/info` 的 `user_id`，本端会在 test 或上传前保存该值，拿不到时按上游 `Init` 语义报错。
...
- `src/components/FileTab.vue` 是主文件管理 Tab，顶部按钮和右键菜单接入上传、下载、新建、重命名、复制、移动、删除。文件选择支持 Ctrl/Shift 多选、Ctrl+A、Esc、Delete、Enter，选中项可批量下载或批量发送到 Motrix Next。文件右键菜单、下载、复制链接、分享、删除确认、路径分组和文档链接生成抽到 `src/utils/file_actions.ts`，Dock 文件树复用同一套动作定义；图片 Viewer、URL 规范化、大小格式化、prompt 和错误提示抽到 `src/utils/file_ui.ts`。后续改菜单、图片预览或文件 UI 公共行为只改这两处。
- `src/components/Dock.vue` 内联 Dock 轻量文件树，按思源 `layout/dock/Files.ts` 文档树 DOM/class 结构渲染：根级每项独立 `ul.b3-list.b3-list--background`，展开子级使用相邻 `ul`，行内保留 `--file-toggle-width`、toggle `padding-left` 和缩进线主题参数 `--QYL-indent-1`；文件树作为 Dock 的直接子级渲染，不经过额外内容包装层，避免和文件树自身左右间距叠加；它只复用 `/api/fs/list` 按需加载目录，并复用 FileTab 的 OpenList 文件图标、共享右键菜单、共享图片 Viewer、拖入文档和 companion `data-href` 链接边界。Dock 文件树删除同样走 `/api/fs/remove` 和二次确认，根目录挂载入口删除会删除对应 storage；不新增边框/颜色样式规则、不新增 kernel route，也不改变 OpenList API surface。不要在稳定渲染树上保留 `file-tree__sliderDown`，该 class 在思源源码中是临时动画态，会让子树 `height: 0`。
...
- Kernel runtime data：`siyuan.storage`，当前拆分为 `/storage/petal/siyuan-cloud/config.json`（可同步配置：settings/users/storages/metas/sharings/ssh_keys）、`runtime.json`（虚拟 FS、tasks、messages、scan、WebDAV locks、S3 multipart 运行态）和 `search-index.json`（搜索索引）。虚拟 FS 根目录由 `state.entries["/"]` 提供，不作为可添加 driver 暴露；历史 `/` `SiYuanKernel` storage 会在加载/导入配置时过滤。旧版 `siyuan-cloud/state.json` 只作为迁移来源读取，不能继续写出双层目录。
