...
- 2026-07-07 S3 follow-up: S3/Doge keeps `read()` as plugin-side signed object reading for `/p` and `/d`, while the new `link()` method serves `/api/fs/link` with OpenList-style GET presigned URLs. This avoids SiYuan Reader/PDF.js Range preview requests receiving upstream 403 responses from a proxied presigned URL.
- 2026-07-10 file interaction/upload follow-up: FileTab supports Ctrl/Shift multi-select, Ctrl+A/Esc/Delete/Enter shortcuts, and batch download/Motrix Next handoff ordered by the visible list. FileTab and Dock drag-to-editor share the same document link helpers; images insert Markdown image links, audio/video insert HTML media tags, and generic files insert `siyuan://plugins/siyuan-cloud/open?path=...` links. S3/Doge upload now tries OpenList `HttpDirect` presigned PUT first, then explicit base64 `/api/fs/put`; WPS upload stays disabled in the JS kernel runtime to avoid blocking SiYuan.
...
- 2026-07-07 follow-up: `189CloudPC` family mode now normalizes `root_folder_id=-11` to an empty family root and auto-fills `family_id` during PC session refresh. The older broad placeholder boundary is now limited to PC password login, AES `params`, upload, family transfer, rapid/CAS, and torrent flows.
...
- Quark/UC request handling mirrors OpenList cookie refresh for `__puus` and Quark transcoding `__pus`, writes updated cookies through `saveDriverStorage`, and exposes `/member` storage details. QuarkOpen saves `/open/v1/user/info` `user_id` before upload proof generation and fails clearly if the upstream user ID is unavailable.
